...
for (m in meth_list) {
  for (d in dist_list){
    best_cluster <- fun_nc(dfn_pca, d, 2, 5, m)
...
fun_clust <- function(data_frame, distance, agglo_method, clusters) {
  ...
  plot(dend1, main = "Hierarchical Clustering using PCA")
...
Step 4: Execute the cluserting using dfn_pca
```{r}
#fun_clust <- function(data_frame, distance, agglo_method, clusters)
pca_max_ward_num <- fun_clust(dfn_pca, "maximum", "ward.D", 5)
```

Evaluate Clusters
```{r}
table(pca_max_ward_num)
```
```{r}
library(ggplot2)
tmp_data <- zdata
tmp_data$cluster <-pca_max_ward_num
tmp_data$cluster <- as.factor(tmp_data$cluster)
p <- ggplot(tmp_data, aes(x=cluster, y=rentZestimate)) + 
  geom_boxplot()
p
p <- ggplot(tmp_data, aes(x=cluster, y=Sch_Rat_Avg)) + 
  geom_boxplot()
p
p <- ggplot(tmp_data, aes(x=cluster, y=Income_per_return)) + 
  geom_boxplot()
p
p <- ggplot(tmp_data, aes(x=cluster, y=violent_crime_total_rate)) + 
  geom_boxplot()
p
...
### We will add autoencoder results to the 5 clusters as part of anamoly detection
```{r}
library("h2o")
h2o.init()
...
```{r}
model=h2o.deeplearning(1:10,
                  training_frame=as.h2o(dfn_pca),
                  hidden=c(10,5,10),
                  epochs = 100,
                  autoencoder = T,
                  activation="Tanh")
...
```{r}
pred.oc <- as.data.frame(h2o.anomaly(model, as.h2o(dfn_pca)))
head(pred.oc)
```

```{r}
tmp_data$ae_mse <-pred.oc$Reconstruction.MSE
```

### Export data to csv
```{r}
write.csv(tmp_data,"cluster_output_5cls.csv", row.names = FALSE)
```
